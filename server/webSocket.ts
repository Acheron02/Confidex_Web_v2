...
async function handleClose(ws: BoothSocket) {
  try {
    ...
    // Do not immediately mark the booth offline on a weak mobile signal.
    // Touch lastSeenAt only; the admin UI derives online/unstable/offline from recency.
    await dbConnect();
...
export function broadcast(data: unknown) {
  ...
  logInfo("[WS] Broadcast sent to", sent, "client(s):", payload);
  return sent > 0;
}
